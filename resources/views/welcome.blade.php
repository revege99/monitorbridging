                    ...
                    <nav class="mt-6 flex-1 overflow-y-auto pr-0">
                        <div class="space-y-0">
                            <div class="-ml-[5px] mb-2">
                                <a href="{{ url('/') }}" class="relative flex items-center gap-2 overflow-hidden rounded-xl px-2 py-2 text-sm font-semibold {{ ($statisticsPage ?? false) ? 'text-slate-300 hover:bg-white/5' : 'bg-gradient-to-r from-blue-600/20 to-blue-500/10 text-white ring-1 ring-blue-400/15' }}">
                                    @unless ($statisticsPage ?? false)
                            ...
                            <section>
                                <div class="rounded-2xl px-0">
                                    ...
                                    <div class="mt-1 space-y-1 pl-[7px]" data-dropdown-panel>
                                        <a href="{{ route('monitoring.statistik-pasien') }}" class="flex items-center gap-2.5 rounded-xl px-2 py-2.5 text-sm {{ ($statisticsPage ?? false) ? 'bg-cyan-400/8 text-white ring-1 ring-cyan-400/10' : 'text-slate-300 hover:bg-white/5' }}">
                                        ...
                                        <a href="{{ route('monitoring.bridging-pelayanan') }}" class="flex items-center gap-2.5 rounded-xl px-2 py-2.5 text-sm {{ ($statisticsPage ?? false) ? 'text-slate-300 hover:bg-white/5' : 'bg-cyan-400/8 text-white ring-1 ring-cyan-400/10' }}">
                                            <span class="h-1.5 w-1.5 rounded-full bg-slate-500"></span>
                                            <svg class="h-5 w-5 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            <main class="bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_25%),linear-gradient(180deg,_#f8fafc_0%,_#eef4fb_100%)]">
                <div class="app-page-header">
                    <div class="flex items-center justify-between px-3 py-2.5 lg:px-3">
                        <div>
                            <h2 class="text-base font-semibold text-slate-900">{{ $pageTitle ?? 'Bridging Pelayanan' }}</h2>
                            <div class="mt-1 flex items-center gap-2 text-xs text-slate-500">
                                <span class="text-cyan-600">Monitor BPJS</span>
                                <span>&gt;</span>
                                <span>{{ $pageTitle ?? 'Bridging Pelayanan' }}</span>
                            </div>
                        </div>
                ...
                <div class="px-3 py-4 lg:px-3">
                    @if ($statisticsPage ?? false)
                        @php
                            $statCards = [
                                ['key' => 'total', 'label' => 'Total Pasien BPJS', 'tone' => 'blue'],
                                ['key' => 'bridging_lengkap', 'label' => 'Bridging Rajal', 'tone' => 'emerald'],
                                ['key' => 'bridging_rujuk', 'label' => 'Bridging Rujuk', 'tone' => 'amber'],
                                ['key' => 'bpjs_batal', 'label' => 'BPJS Batal', 'tone' => 'rose'],
                                ['key' => 'tidak_bridging', 'label' => 'Belum Bridging', 'tone' => 'rose'],
                            ];
                            $toneClasses = [
                        ...
                        <div class="grid gap-2.5 md:grid-cols-2 lg:grid-cols-5">
                            @foreach ($statCards as $card)
                                <div class="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
                                    <div class="flex items-start gap-3">
                                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl {{ $toneClasses[$card['tone']] }}">
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M16 21v-2a4 4 0 0 0-4-4H7a4 4 0 0 0-4 4v2" />
                                                <circle cx="9.5" cy="7" r="3" />
                                                <path d="M20 21v-2a4 4 0 0 0-3-3.87" />
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <p class="mt-1 text-2xl font-semibold text-slate-950">{{ number_format((int) ($summary[$card['key']] ?? 0)) }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        @if ($usingFallback)
                            <div class="mt-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                                Statistik belum dapat diambil dari database klinik.
                                @if ($dbError)
                        ...
                        <div class="mt-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
                            <form method="GET" action="{{ route('monitoring.statistik-pasien') }}" class="flex flex-nowrap items-end gap-2">
                                ...
                                <div class="flex shrink-0 items-end gap-2">
                                    <button type="submit" class="inline-flex h-[35px] cursor-pointer items-center justify-center rounded-2xl bg-blue-600 px-4 text-xs font-semibold text-white transition hover:bg-blue-700">Filter</button>
                                    <a href="{{ route('monitoring.statistik-pasien') }}" class="inline-flex h-[35px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">Reset</a>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="mt-2.5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                            <div class="border-b border-slate-100 px-3 py-3">
                                <h3 class="text-sm font-semibold text-slate-900">Rekap Berdasarkan Tanggal Registrasi</h3>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="min-w-full text-xs">
                                    <thead class="bg-slate-50 text-slate-600">
                                        <tr>
                                            <th class="px-3 py-3.5 text-left font-semibold">Tanggal Registrasi</th>
                                            <th class="px-3 py-3.5 text-center font-semibold">Total BPJS</th>
                                            <th class="px-3 py-3.5 text-center font-semibold">Rujuk</th>
                                            <th class="px-3 py-3.5 text-center font-semibold">BPJS Batal</th>
                                            <th class="px-3 py-3.5 text-center font-semibold">Belum Bridging</th>
                                            <th class="px-3 py-3.5 text-center font-semibold">Bridging Rajal</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100">
                                        @forelse ($dailyRows as $row)
                                            <tr class="hover:bg-slate-50/80">
                                                <td class="px-3 py-3.5 font-medium text-slate-900">{{ $row->tgl_registrasi }}</td>
                                                <td class="px-3 py-3.5 text-center font-semibold text-slate-900">{{ $row->total }}</td>
                                                <td class="px-3 py-3.5 text-center text-slate-700">{{ $row->rujuk }}</td>
                                                <td class="px-3 py-3.5 text-center text-slate-700">{{ $row->bpjs_batal }}</td>
                                                <td class="px-3 py-3.5 text-center text-rose-600">{{ $row->tidak_bridging }}</td>
                                                <td class="px-3 py-3.5 text-center text-emerald-600">{{ $row->bridging_lengkap }}</td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="6" class="px-3 py-8 text-center text-slate-500">Tidak ada statistik pasien pada periode yang dipilih.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                    ...
                    <div class="grid gap-2.5 md:grid-cols-2 xl:grid-cols-4">
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Total Bridging Hari Ini</p>
                        ...
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Bridging H+1</p>
                    ...
                    @if ($usingFallback)
                        <div class="mt-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                            Data masih memakai fallback lokal karena koneksi database klinik belum berhasil.
                            @if ($dbError)
                                <div class="mt-1 font-mono text-[11px]">{{ $dbError }}</div>
                            @endif
                        </div>
                    ...
                    <div class="mt-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
                        <form method="GET" action="{{ url('/') }}" class="grid gap-2 lg:grid-cols-[minmax(0,1.3fr)_repeat(2,minmax(0,0.8fr))_minmax(0,0.9fr)_auto]">
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">Cari Pasien</label>
                                <input type="text" value="" placeholder="Cari nama / no rawat / no RM..." data-live-search data-search-empty-message="Tidak ada data bridging yang cocok." class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                            </div>
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">Tanggal Awal</label>
                                <input type="date" name="start_date" value="{{ $filters['start_date'] }}" class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                            ...
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">Status Bridging</label>
                                <select name="status_bridging" class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                                    <option value="all" @selected($filters['status_bridging'] === 'all')>Semua</option>
                                    <option value="same_day" @selected($filters['status_bridging'] === 'same_day')>Same Day</option>
                                    <option value="cross_day" @selected($filters['status_bridging'] === 'cross_day')>H+1 / Lintas Hari</option>
                                    <option value="not_bridged" @selected($filters['status_bridging'] === 'not_bridged')>Belum Bridging</option>
                                </select>
                            </div>
                            <div class="flex items-end gap-2">
                                ...
                                <a href="{{ url('/') }}" class="inline-flex h-[35px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">
                                    Reset
                                </a>
                            </div>
                        </form>
                            ...
                            <table class="min-w-full text-left text-xs">
                                ...
                                <thead class="bg-slate-50 text-slate-600">
                                    <tr>
                                        ...
                                        <th class="px-3 py-3.5 font-semibold">Bridging Pendaftaran</th>
                                        <th class="px-3 py-3.5 font-semibold">Bridging Kunjungan</th>
                                        <th class="px-3 py-3.5 font-semibold">Status</th>
                                        <th class="px-3 py-3.5 font-semibold">Catatan</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    @foreach ($bridgingRows as $index => $row)
                                        <tr class="hover:bg-slate-50/80" data-search-row data-search-text="{{ strtolower($row['no_rawat'].' '.$row['norm'].' '.$row['pasien'].' '.$row['poli'].' '.$row['status'].' '.$row['catatan']) }}">
                                            ...
                                            <td class="px-3 py-3.5">
                                                <span class="@if($row['warna'] === 'emerald') bg-emerald-50 text-emerald-700 @elseif($row['warna'] === 'amber') bg-amber-50 text-amber-700 @else bg-rose-50 text-rose-700 @endif inline-flex rounded-full px-3 py-1 text-[11px] font-semibold">
                                                    {{ $row['status'] }}
                                                </span>
                                            </td>
                                            <td class="px-3 py-3.5 text-slate-600">{{ $row['catatan'] }}</td>
                                        </tr>
                                    @endforeach
                                    <tr class="hidden" data-search-empty-row>
                                        <td colspan="9" class="px-3 py-8 text-center text-slate-500">Tidak ada data bridging yang cocok.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
