                            ...
                            <section>
                                <div class="rounded-2xl px-0">
                                    ...
                                    <div class="mt-1 space-y-1 pl-[7px]" data-dropdown-panel>
                                        ...
                                        <a href="{{ route('monitoring.bpjs-tidak-bridging') }}" class="flex items-center gap-2.5 rounded-xl bg-cyan-400/8 px-2 py-2.5 text-sm text-white ring-1 ring-cyan-400/10">
                                            <span class="h-1.5 w-1.5 rounded-full bg-slate-500"></span>
                                            <svg class="h-5 w-5 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M5 6h14" />
                                                <path d="M7 6v12" />
            ...
            <main class="bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_25%),linear-gradient(180deg,_#f8fafc_0%,_#eef4fb_100%)]">
                <div class="app-page-header">
                    <div class="flex items-center justify-between px-3 py-2.5 lg:px-3">
                        <div>
                            <h2 class="text-base font-semibold text-slate-900">Rujukan Non Spesialistik</h2>
                ...
                <div class="px-3 py-4 lg:px-3">
                    <div class="grid gap-2.5 md:grid-cols-2 xl:grid-cols-4">
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Total Kasus</p>
                        ...
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Dengan Diagnosa</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $summary['dengan_diagnosa'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Tanpa TACC</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $summary['tanpa_tacc'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Ada Estimasi Rujuk</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $summary['dengan_estimasi'] }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($usingFallback)
                        <div class="mt-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                            Data rujukan non spesialistik masih memakai fallback lokal karena koneksi database klinik belum berhasil.
                            @if ($dbError)
                    ...
                    <div class="mt-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
                        <form method="GET" action="{{ route('monitoring.bpjs-tidak-bridging') }}" class="flex flex-nowrap items-end gap-2">
                            <div class="shrink-0" style="width: 29.5%;">
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">Cari Pasien</label>
                                <input type="text" value="" placeholder="Cari nama / no rawat / no RM / diagnosa..." data-live-search data-search-empty-message="Tidak ada data rujukan non spesialistik yang cocok." class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                            </div>
                            ...
                            <div class="flex shrink-0 items-end justify-end gap-2">
                                ...
                                <a href="{{ route('monitoring.bpjs-tidak-bridging') }}" class="inline-flex h-[35px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">
                                    Reset
                                </a>
                            </div>
                        </form>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-left text-xs">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse ($nonSpesialisRows as $index => $row)
                                        <tr class="hover:bg-slate-50/80" data-search-row data-search-text="{{ strtolower($row['no_rawat'].' '.$row['norm'].' '.$row['pasien'].' '.$row['poli'].' '.$row['diagnosa'].' '.$row['kode_diagnosa'].' '.$row['ppk_rujuk'].' '.$row['subspesialis'].' '.$row['tacc'].' '.$row['status_fraud']) }}">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="px-3 py-8 text-center text-slate-500">
                                                Tidak ada data rujukan non spesialistik pada filter yang dipilih.
                                            </td>
                                        </tr>
                                    @endforelse
                                    <tr class="hidden" data-search-empty-row>
                                        <td colspan="10" class="px-3 py-8 text-center text-slate-500">Tidak ada data rujukan non spesialistik yang cocok.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
