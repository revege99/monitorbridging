            ...
            <main class="bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_25%),linear-gradient(180deg,_#f8fafc_0%,_#eef4fb_100%)]">
                <div class="app-page-header">
                    <div class="flex items-center justify-between px-3 py-2.5 lg:px-3">
                        <div>
                            <h2 class="text-base font-semibold text-slate-900">Bridging Rujuk</h2>
                            <div class="mt-1 flex items-center gap-2 text-xs text-slate-500">
                                <span class="text-cyan-600">Monitor BPJS</span>
                                <span>&gt;</span>
                                <span>Bridging Rujuk</span>
                            </div>
                        </div>
                ...
                <div class="px-3 py-4 lg:px-3">
                    <div class="grid gap-2.5 md:grid-cols-2 xl:grid-cols-4">
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Total Bridging Rujuk</p>
                        ...
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Non Spesialis</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $summary['non_spesialis'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">Dengan TACC</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $summary['dengan_tacc'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                    ...
                    @if ($usingFallback)
                        <div class="mt-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                            Data pasien rujuk masih memakai fallback lokal karena koneksi database klinik belum berhasil.
                            @if ($dbError)
                    ...
                    <div class="mt-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
                        <form method="GET" action="{{ route('monitoring.pasien-rujuk') }}" class="grid gap-2 lg:grid-cols-[minmax(0,1.4fr)_repeat(2,minmax(0,0.8fr))_auto]">
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">Cari Pasien</label>
                                <input type="text" value="" placeholder="Cari nama / no rawat / no RM / diagnosa..." data-live-search data-search-empty-message="Tidak ada data pasien rujuk yang cocok." class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                            </div>
                            <div>
                            ...
                            <div class="flex items-end gap-2">
                                ...
                                <a href="{{ route('monitoring.pasien-rujuk') }}" class="inline-flex h-[35px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">
                                    Reset
                                </a>
                            </div>
                        </form>
                            ...
                            <table class="min-w-full text-left text-xs">
                                ...
                                <thead class="bg-slate-50 text-slate-600">
                                    <tr>
                                        ...
                                        <th class="px-3 py-3.5 font-semibold">Non Spesialis</th>
                                        <th class="px-3 py-3.5 font-semibold">Tujuan Rujuk</th>
                                        <th class="px-3 py-3.5 font-semibold">TACC</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    @forelse ($rujukRows as $index => $row)
                                        <tr class="hover:bg-slate-50/80" data-search-row data-search-text="{{ strtolower($row['no_rawat'].' '.$row['norm'].' '.$row['pasien'].' '.$row['poli'].' '.$row['diagnosa'].' '.$row['kode_diagnosa'].' '.$row['non_spesialis'].' '.$row['subspesialis'].' '.$row['ppk_rujuk'].' '.$row['tacc'].' '.$row['alasan_tacc']) }}">
                                            ...
                                            <td class="px-3 py-3.5 text-slate-700">{{ $row['non_spesialis'] }}</td>
                                            <td class="px-3 py-3.5 text-slate-700">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="px-3 py-8 text-center text-slate-500">
                                                Tidak ada data pasien rujuk pada filter yang dipilih.
                                            </td>
                                        </tr>
                                    @endforelse
                                    <tr class="hidden" data-search-empty-row>
                                        <td colspan="10" class="px-3 py-8 text-center text-slate-500">Tidak ada data pasien rujuk yang cocok.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
