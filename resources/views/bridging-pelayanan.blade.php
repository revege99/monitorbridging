...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-[#f4f7fb] text-slate-900">
        @php
            $isRujuk = $filters['jenis'] === 'rujuk';
            $baseTabQuery = request()->except('status_bridging', 'status_rujuk');
            $rajalQuery = array_merge($baseTabQuery, ['jenis' => 'rajal']);
            $rujukQuery = array_merge($baseTabQuery, ['jenis' => 'rujuk']);
        @endphp
                    ...
                    <nav class="mt-6 flex-1 overflow-y-auto pr-0">
                        <div class="space-y-0">
                            ...
                            <section>
                                <div class="rounded-2xl px-0">
                                    ...
                                    <div class="mt-1 space-y-1 pl-[7px]" data-dropdown-panel>
                                        ...
                                        <a href="{{ route('monitoring.bridging-pelayanan') }}" class="flex items-center gap-2.5 rounded-xl bg-cyan-400/8 px-2 py-2.5 text-sm text-white ring-1 ring-cyan-400/10">
                                            <span class="h-1.5 w-1.5 rounded-full bg-slate-500"></span>
                                            <svg class="h-5 w-5 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <section>
                                <div class="rounded-2xl px-0">
                                    <button type="button" class="flex w-full cursor-pointer items-center gap-2.5 rounded-2xl px-0 py-1.5 text-left text-white" data-dropdown-toggle aria-expanded="true">
                                        <span class="flex h-11 w-11 items-center justify-center text-fuchsia-200">
                                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M12 2v4" />
                                                <path d="M12 18v4" />
            ...
            <main class="bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.08),_transparent_25%),linear-gradient(180deg,_#f8fafc_0%,_#eef4fb_100%)]">
                ...
                <div class="px-3 py-4 lg:px-3">
                    ...
                    <div class="mt-1 grid gap-2.5 {{ $isRujuk ? 'grid-cols-3' : 'grid-cols-4' }}">
                        ...
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                            <div class="flex items-start gap-3">
                                <div class="flex h-10 w-10 items-center justify-center rounded-2xl {{ $isRujuk ? 'bg-emerald-50 text-emerald-600' : 'bg-emerald-50 text-emerald-600' }}">
                                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                        <circle cx="12" cy="12" r="8" />
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">{{ $isRujuk ? 'Non Spesialis' : 'Same Day' }}</p>
                                    <p class="mt-2 text-2xl font-semibold text-slate-950">{{ $isRujuk ? $summary['non_spesialis'] : $summary['same_day'] }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm">
                    ...
                    @if ($usingFallback)
                        <div class="mt-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                            Data {{ $isRujuk ? 'bridging rujuk' : 'bridging rajal' }} masih memakai fallback lokal karena koneksi database klinik belum berhasil.
                            @if ($dbError)
                    ...
                    <div class="mt-2.5 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
                        <form method="GET" action="{{ url('/') }}" class="grid gap-2 lg:grid-cols-[minmax(0,1.3fr)_repeat(2,minmax(0,0.8fr))_minmax(0,0.9fr)_auto]">
                            ...
                            <div>
                                ...
                                <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="{{ $isRujuk ? 'Cari nama / no rawat / diagnosa...' : 'Cari nama / no rawat / no RM...' }}" class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-slate-700">{{ $isRujuk ? 'Status Rujuk' : 'Status Bridging' }}</label>
                                @if ($isRujuk)
                                    <select name="status_rujuk" class="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100">
                                        <option value="all" @selected($filters['status_rujuk'] === 'all')>Semua</option>
                                        <option value="dengan_tacc" @selected($filters['status_rujuk'] === 'dengan_tacc')>Dengan TACC</option>
                                        <option value="tanpa_tacc" @selected($filters['status_rujuk'] === 'tanpa_tacc')>Tanpa TACC</option>
                                    </select>
                                @else
                            ...
                            <div class="flex items-end gap-2">
                                ...
                                <a href="{{ url('/?' . http_build_query(['jenis' => $filters['jenis']])) }}" class="inline-flex h-[35px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">
                                    Reset
                                </a>
                            </div>
                        </form>
                            ...
                            <table class="min-w-full text-left text-xs">
                                ...
                                <thead class="bg-slate-50 text-slate-600">
                                    <tr>
                                        ...
                                        @if ($isRujuk)
                                            <th class="px-3 py-3.5 font-semibold">Tanggal Rujuk</th>
                                            <th class="px-3 py-3.5 font-semibold">Diagnosa</th>
                                            <th class="px-3 py-3.5 font-semibold">Tujuan Rujuk</th>
                                            <th class="px-3 py-3.5 font-semibold">TACC</th>
                                        @else
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse ($rows as $index => $row)
                                        <tr class="hover:bg-slate-50/80">
                                            ...
                                            @if ($isRujuk)
                                                <td class="px-3 py-3.5">
                                                    <p class="font-medium text-slate-900">{{ $row['tgl_rujuk'] ?? '-' }}</p>
                                                    <p class="mt-1 text-xs text-slate-500">Estimasi {{ $row['tgl_estimasi_rujuk'] ?? '-' }}</p>
                                                </td>
                                                <td class="px-3 py-3.5 text-slate-700">
                                                    <p class="font-medium text-slate-900">{{ $row['diagnosa'] }}</p>
                                                    <p class="mt-1 text-xs text-slate-500">{{ $row['kode_diagnosa'] }}</p>
                                                </td>
                                                <td class="px-3 py-3.5 text-slate-700">
                                                    <p class="font-medium text-slate-900">{{ $row['subspesialis'] }}</p>
                                                    <p class="mt-1 text-xs text-slate-500">{{ $row['ppk_rujuk'] }}</p>
                                                </td>
                                                <td class="px-3 py-3.5 text-slate-700">
                                                    <p class="font-medium text-slate-900">{{ $row['tacc'] }}</p>
                                                    <p class="mt-1 text-xs text-slate-500">{{ $row['alasan_tacc'] }}</p>
                                                </td>
                                            @else
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="px-3 py-8 text-center text-slate-500">
                                                Tidak ada data {{ $isRujuk ? 'bridging rujuk' : 'bridging rajal' }} pada filter yang dipilih.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
